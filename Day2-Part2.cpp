...
bool checkPassword(string range, char charCheck, string pw){
    ...
    string v1, v2;
    char checkFor;
    int begRange=stoi(range.substr(0,range.find(delim)))-1;
    int endRange=stoi(range.substr(range.find(delim)+1,range.length()))-1;
    if(pw.at(begRange)==charCheck && pw.at(endRange)!=charCheck){
        return true;
    }
    else if(pw.at(begRange)!=charCheck && pw.at(endRange)==charCheck){
        return true;
    }
    else{
        return false;
